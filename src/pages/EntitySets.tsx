...
const EntitySets = () => {
  ...
  useEffect(() => {
    const fetchEntitySets = async () => {
      try {
        setIsLoading(true);
        const entitySetsQuery = query(
          collection(db, 'entitySets'),
  ...
  const handleAddEntityArray = () => {
    if (entityArray.trim()) {
      try {
        ...
        if (
          entityArray.trim().startsWith('[') &&
          entityArray.trim().endsWith(']')
        ) {
        ...
        toast({
          ...
          description:
            'Please provide a valid JSON array or comma-separated list',
          variant: 'destructive',
        });
      }
    }
  ...
  const handleRemoveEntity = (entityToRemove: string) => {
    setEntitiesForNewSet(
      entitiesForNewSet.filter((entity) => entity !== entityToRemove)
    );
  };
  ...
  const handleCreateEntitySet = async () => {
    if (newSetName.trim() && entitiesForNewSet.length > 0) {
      ...
      toast({
        ...
        description:
          'Please provide a name and at least one entity for the set.',
        variant: 'destructive',
      });
    }
  };
  ...
  return (
    <div>
      <DashboardHeader
        title="Entity Sets"
        subtitle="Manage your document entity extraction sets"
      />
      ...
      <div className="p-6">
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-500 text-sm">
              Create and manage sets of entities for document extraction
            </p>
          </div>
          ...
          >
            ...
            <DialogContent className="sm:max-w-lg">
              <DialogHeader>
                <DialogTitle>
                  {editingSet ? 'Edit Entity Set' : 'Create New Entity Set'}
                </DialogTitle>
                <DialogDescription>
                  {editingSet
              ...
              <div className="grid gap-4 py-4">
                ...
                <div className="grid grid-cols-4 items-start gap-4">
                  ...
                  <div className="col-span-3 space-y-2">
                    ...
                    <p className="text-xs text-gray-500">
                      Add multiple entities using JSON array or comma-separated
                      values
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-4 items-start gap-4">
                  <Label className="text-right font-medium">Entities</Label>
                      ...
                      <div className="flex flex-wrap gap-2 p-3 border border-gray-200 rounded-md bg-gray-50">
                        {entitiesForNewSet.map((entity, index) => (
                          <Badge
                            key={index}
                            variant="secondary"
                            className="px-2 py-1 flex gap-1 items-center bg-white border border-gray-200"
                          >
                            {entity}
                            <button
                              type="button"
        ...
        {isLoading ? (
          <div className="flex justify-center items-center p-12">
            ...
            <span className="ml-2 text-lg text-[#13294B]">
              Loading entity sets...
            </span>
          </div>
        ) : (
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {entitySets.map((set) => (
                <Card
                  key={set.id}
                  className="border border-gray-200 transition-shadow hover:shadow-md overflow-hidden"
                >
                  <CardHeader className="pb-3 bg-gray-50 border-b">
                    <div className="flex justify-between items-start">
                      <div>
                        <CardTitle className="text-[#13294B]">
                          {set.name}
                        </CardTitle>
                        <CardDescription>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {set.entities.map((entity, index) => (
                        <Badge
                          key={index}
                          variant="outline"
                          className="px-2 py-1 border-[#13294B]/30 bg-[#13294B]/5 text-[#13294B]"
                        >
                          {entity}
                        </Badge>
                      ))}
            ...
            {entitySets.length === 0 && (
              <div className="bg-white rounded-lg border border-gray-200 p-8 text-center">
                ...
                <p className="text-gray-500 mb-4">
                  Create your first entity set to start extracting information
                  from your documents.
                </p>
                <Button
